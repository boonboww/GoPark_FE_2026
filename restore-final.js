...
import { Plus, Edit2, Trash2, Camera, Car, Info, QrCode, Mail } from "lucide-react";
...
// đây là interface tạm thời để tránh lỗi, sau này sẽ đồng bộ với backend
...
export default function ProfilePage() {
  ...
  const openAddVehicle = () => {
    if ((vehicles?.length || 0) >= MAX_VEHICLES) {
      toast.error(\Bạn chỉ được đăng ký tối đa \ phương tiện!\);
      return;
    }
  ...
  const handleDeleteVehicle = async (id: string) => {
    if (confirm("Bạn có chắc chắn muốn xóa phương tiện này?")) {
      try {
        await apiClient(\/vehicles/\\, { method: "DELETE" });
  ...
  const handleSaveVehicle = async () => {
    ...
    try {
      if (editingVehicleId) {
        ...
        // Add new
        if ((vehicles?.length || 0) >= MAX_VEHICLES) {
          toast.error(\Chỉ có thể thêm tối đa \ phương tiện.\);
          return;
        }
        const res = await apiClient<any>("/vehicles", {
          method: "POST",
        ...
        <div className="col-span-1 lg:col-span-7">
          <Card className="shadow-sm border-blue-50/50 h-full">
            <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <Button onClick={openAddVehicle} disabled={(vehicles?.length || 0) >= MAX_VEHICLES} size="sm" className="w-full bg-green-800 cursor-pointer sm:w-auto">
            ...
            <CardContent>
              {(!vehicles || vehicles.length === 0) ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  {(vehicles || []).map((v) => (
                    <div key={v.id} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl border border-slate-200 hover:border-blue-300 transition-colors bg-white group">
                        ...
                        <div>
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex items-center gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                              <Button variant="ghost" size="icon" onClick={() => openEditVehicle(v)} className="h-8 w-8 text-blue-600 hover:bg-blue-50">
                                <Edit2 className="w-4 h-4" />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleDeleteVehicle(v.id)} className="h-8 w-8 text-red-600 hover:bg-red-50">
...
fs.writeFileSync('src/app/users/profile/page.tsx', content, 'utf8');
