...
function historyBooking(){
  ...
  const parkingOptions = Array.from(
    new Set(booking.map(b => b.name))
  );
  ...
  return (
    <div className="min-h-screen bg-[#F8FBFF] text-slate-900 font-sans pb-16">
      ...
      <main className="max-w-[1200px] mx-auto px-6 mt-12">
        <div className="mb-10">
          <h2 className="text-4xl font-black tracking-tight mb-2">Lịch sử đặt chỗ</h2>
          <p className="text-slate-600 font-medium text-base">Xem và quản lý lịch sử đỗ xe của bạn</p>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-6 lg:p-8 shadow-sm border border-slate-200 mb-10">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-slate-900 tracking-tight">Tìm kiếm chuyến đi</h3>
          </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-end gap-6 lg:gap-4">
            ...
            <div className="w-full lg:w-[280px]">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-3">Khoảng thời gian</label>
            ...
            <div className="w-full lg:w-[350px] lg:mx-auto">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-3">Tìm kiếm</label>
              <div className="relative w-full">
                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500">
            ...
            <div className="w-full lg:w-auto lg:border-l border-slate-200 lg:pl-8 flex flex-col items-start lg:items-end pb-[2px]">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-3">Gợi ý nhanh</label>
              <button
                onClick={() => {
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between mb-8 gap-5 px-1 w-full">
          ...
          <div className="flex items-center bg-white rounded-full p-1.5 shadow-sm border border-slate-200 overflow-x-auto w-full lg:w-auto shrink-0 z-10">
            {statusOptions.map(status => {
              ...
              const matchStatus = status == "Tất cả" ? true : b.status == status;
              const matchDate = !selectedDate ? true : b.start_date_iso == selectedDate;
          ...
          <div className="flex bg-blue-600 text-white text-base font-bold px-6 py-3 rounded-xl border border-blue-700 items-center justify-center gap-3 shadow-md w-full lg:w-auto h-[52px] shrink-0">
             <span className="uppercase tracking-wider text-blue-50">{filterStatus === "Tất cả" ? "Tổng chuyến đi" : filterStatus}</span>
             <span className="bg-white px-3.5 py-1.5 rounded-lg shadow-sm text-xl text-blue-700 min-w-[36px] text-center leading-none flex items-center justify-center">{filteredBooking.length}</span>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div key={item.id} className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-200 flex flex-col lg:flex-row gap-8 items-stretch hover:shadow-md transition-shadow cursor-default">
            ...
            <div className="flex-1 flex flex-col justify-between py-2 pr-2">
              <div>
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-2xl font-black text-slate-900 mb-2.5 tracking-tight">{item.name}</h3>
                  ...
                  <div className={`text-xs font-bold tracking-widest px-5 py-2.5 rounded-full uppercase text-white ${
                      item.status === 'Đã đặt' 
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                  <div>
                    <p className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">Thời gian vào</p>
                    <div className="flex flex-col gap-1">
                      <span className="font-bold text-slate-900 text-[19px] leading-tight">{item.start_time}</span>
                      <span className="font-semibold text-slate-600 text-sm">{item.start_date}</span>
                    </div>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">Thời gian ra</p>
                    <div className="flex flex-col gap-1">
                      <span className="font-bold text-slate-900 text-[19px] leading-tight">{item.end_time}</span>
                      <span className="font-semibold text-slate-600 text-sm">{item.end_date}</span>
                    </div>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-600 uppercase tracking-widest mb-1.5">Vị trí đỗ</p>
                    <div className="flex flex-col gap-1 mt-1">
                      <span className="font-black text-blue-700 text-[20px] leading-tight bg-blue-100/60 w-fit px-3 py-1.5 rounded-lg border border-blue-200/50">{item.code}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-between items-end mt-2">
                <div className="flex flex-col gap-1 px-2">
                  <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Tổng tiền</p>
                  <span className="font-black text-red-600 text-[26px] leading-none">
                    {item.total_price ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.total_price) : "0 ₫"}
                  </span>
                </div>
            ...
            <div className="py-20 text-center bg-white rounded-[2rem] border border-slate-200">
               ...
               <h4 className="text-2xl font-bold text-slate-800 mb-2">Không tìm thấy chuyến đi nào</h4>
               <p className="text-slate-600 text-base">Vui lòng thử bộ lọc hoặc từ khóa khác.</p>
            </div>
          )}
        </div>
