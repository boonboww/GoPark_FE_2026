...
export default function ContactPage() {
    ...
    const faqs = [
        ...
        },
        {
            ...
            a: "GoPark hỗ trợ nhiều phương thức thanh toán: Ví GoPark, Thẻ ngân hàng, VN Pay hoặc tiền mặt tại bãi đỗ."
        },
        {
        ...
        {
            q: "Bãi đỗ xe có an toàn không?",
            a: "Tất cả các bãi đỗ trên GoPark đều được kiểm duyệt kỹ lưỡng, có camera giám sát và nhân viên bảo vệ 24/7 để đảm bảo an toàn tuyệt đối cho phương tiện của bạn."
        },
        {
            q: "Làm sao để trở thành đối tác của GoPark?",
            a: "Nếu bạn sở hữu bãi đỗ xe, hãy nhấn vào mục 'Hợp tác' trên website hoặc liên hệ hotline để được đội ngũ của chúng tôi hỗ trợ tích hợp hệ thống."
        },
        {
            q: "Tôi có thể gia hạn thêm thời gian đỗ không?",
            a: "Có, bạn có thể vào mục 'Đặt chỗ của tôi' và chọn 'Gia hạn' nếu chỗ đỗ đó vẫn còn trống trong khung giờ tiếp theo."
        },
        {
            q: "Chính sách hoàn tiền của GoPark như thế nào?",
            a: "Tiền sẽ được hoàn trả vào Ví GoPark hoặc tài khoản gốc của bạn trong vòng 3-5 ngày làm việc nếu việc hủy đặt chỗ tuân thủ đúng quy định."
        }
    ];
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-stone-950 font-sans selection:bg-green-100 selection:text-green-900">
            ...
            <section className="py-32 bg-gray-50 dark:bg-stone-900/30">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-20">
                        ...
                        <p className="text-gray-500 mt-6 max-w-2xl mx-auto font-medium">Giải đáp nhanh những thắc mắc phổ biến nhất để bạn có trải nghiệm đỗ xe tuyệt vời nhất.</p>
                    </div>

                    <div className="flex flex-col lg:flex-row items-start gap-6 max-w-7xl mx-auto">
                        {/* Cột 1 */}
                        <div className="flex-1 w-full space-y-6">
                            {faqs.filter((_, idx) => idx % 2 === 0).map((faq, originalIdx) => (
                                <FaqItem key={originalIdx} faq={faq} idx={originalIdx * 2} openFaq={openFaq} setOpenFaq={setOpenFaq} />
                            ))}
                        </div>
                        {/* Cột 2 */}
                        <div className="flex-1 w-full space-y-6">
                            {faqs.filter((_, idx) => idx % 2 !== 0).map((faq, originalIdx) => (
                                <FaqItem key={originalIdx} faq={faq} idx={originalIdx * 2 + 1} openFaq={openFaq} setOpenFaq={setOpenFaq} />
                            ))}
                        </div>
                    </div>
                </div>
            ...
            {/* CONTACT FORM & INFO */}
            <section className="pb-32 pt-20 container mx-auto px-4">
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <ContactInfoCard
                            icon={MapPin}
                            title="Văn phòng"
                            value="O3 Quang Trung, Đà Nẵng"
                            subValue="Quận Hải Châu"
                        />
...
const FaqItem = ({ faq, idx, openFaq, setOpenFaq }: { faq: any; idx: number; openFaq: number | null; setOpenFaq: (idx: number | null) => void }) => (
    <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ delay: idx * 0.05 }}
        className="bg-white dark:bg-stone-900 rounded-[2.5rem] border border-gray-100 dark:border-stone-800 overflow-hidden shadow-sm hover:shadow-md transition-shadow h-fit"
    >
        <button
            className="w-full p-8 flex items-center justify-between text-left"
            onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
        >
            <span className="text-lg font-black text-gray-900 dark:text-white pr-8">{faq.q}</span>
            <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${openFaq === idx ? 'bg-green-600 text-white' : 'bg-gray-100 dark:bg-stone-800 text-gray-400'}`}>
        ...
        {openFaq === idx && (
            <div className="px-8 pb-8 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium leading-relaxed text-base">
                    {faq.a}
                </p>
            </div>
        )}
