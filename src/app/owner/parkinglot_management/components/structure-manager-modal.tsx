...
export function StructureManagerTab({ onClose }: { onClose?: () => void }) {
  ...
  >({});
  const [zoneForm, setZoneForm] = React.useState<
    Record<string, string | number>
  >({});
  ...
  const [newZoneForm, setNewZoneForm] = React.useState<
    ZonePayload & {
      priceHour: number;
      priceDay: number;
    }
  ...
  const createFloorMut = useMutation({
    mutationFn: () =>
      parkingService.createFloor(lotId as number, {
        floor_name: newFloorForm.floor_name,
        floor_number: newFloorForm.floor_number,
        description:
          newFloorForm.description || `Khu vực ${newFloorForm.floor_name}`,
      }),
    onSuccess: () => {
  ...
  const createZoneMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        "✅ Thêm khu vực, thiết lập giá & khởi tạo slots thành công!",
      );
      invalidate();
      // Invalidate zone slots cache trên trang chính
      queryClient.invalidateQueries({ queryKey: ["zoneSlots"] });
  ...
  const makeGenerateToast = (res: any) => {
    ...
    if (added === 0 && disabled === 0)
      toast.info("ℹ️ Slots đã đồng bộ, không có thay đổi.");
  };
  ...
  const genLotMut = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.response?.data?.message ?? "Lỗi sync lot"),
  });

  const genFloorMut = useMutation({
    mutationFn: (floorId: number) =>
      parkingService.generateSlotsForFloor(lotId as number, floorId),
    onSuccess: (res) => {
      makeGenerateToast(res);
      queryClient.invalidateQueries({ queryKey: ["zoneSlots"] });
    },
    onError: (err: any) =>
      toast.error(err?.response?.data?.message ?? "Lỗi sync floor"),
  });
  ...
  const genZoneMut = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.response?.data?.message ?? "Lỗi sync zone"),
  });
      ...
      <div className="px-6 py-5 border-b bg-white flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-xl font-bold text-slate-800">
            Quản lý Cấu trúc Bãi đỗ
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Thêm, sửa tầng và khu vực đang hoạt động
          </p>
        </div>
        ...
        {addingFloor && (
          <div className="bg-white border-2 border-black/10 border-dashed rounded-2xl p-5 shadow-sm animate-in slide-in-from-top-2 duration-300">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-500 uppercase font-semibold">
                  Tên tầng <span className="text-red-400">*</span>
                </Label>
                <Input
            ...
            <div className="flex justify-end gap-3 mt-4 pt-4 border-t">
              <Button variant="ghost" onClick={() => setAddingFloor(false)}>
                Huỷ
              </Button>
              <Button
                onClick={() => createFloorMut.mutate()}
                disabled={
                  createFloorMut.isPending || !newFloorForm.floor_name.trim()
                }
                className="bg-black hover:bg-slate-800 text-white font-semibold"
              >
                {createFloorMut.isPending ? (
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-slate-400">
            ...
            <p className="font-medium">
              Chưa có tầng nào. Hãy thêm tầng đầu tiên!
            </p>
          </div>
        ) : (
                  ...
                  <div className="px-5 pb-4 pt-2 bg-amber-50/60 border-t border-amber-100 animate-in slide-in-from-top-1 duration-200">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={floorForm.floor_name as string}
                          onChange={(e) =>
                            setFloorForm((p) => ({
                              ...p,
                              floor_name: e.target.value,
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={floorForm.description as string}
                          onChange={(e) =>
                            setFloorForm((p) => ({
                              ...p,
                              description: e.target.value,
                    ...
                    <div className="flex gap-2 mt-3 justify-end">
                      ...
                      <Button
                        size="sm"
                        disabled={updateFloorMut.isPending}
                        onClick={() =>
                          updateFloorMut.mutate({
                            id: floor.id,
                            ...floorForm,
                          } as any)
                        }
                  ...
                  <div className="border-t divide-y bg-slate-50/50">
                          ...
                          <div className="animate-in slide-in-from-top-1 duration-200">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  value={zoneForm.zone_name as string}
                                  onChange={(e) =>
                                    setZoneForm((p) => ({
                                      ...p,
                              ...
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  value={zoneForm.description as string}
                                  onChange={(e) =>
                                    setZoneForm((p) => ({
                                      ...p,
                                      description: e.target.value,
                              ...
                              <div className="space-y-1.5">
                                <Label className="text-xs text-slate-500">
                                  Tiền tố (1-2 ký tự HOA)
                                </Label>
                                <Input
                                  value={zoneForm.prefix as string}
                                  maxLength={2}
                              ...
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  type="number"
                                  value={zoneForm.total_slots as number}
                                  onChange={(e) =>
                                    setZoneForm((p) => ({
                                      ...p,
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-3 p-3 bg-green-50/80 rounded-xl border border-green-100">
                              <div className="space-y-1.5">
                                <Label className="text-xs text-green-700 font-semibold flex items-center gap-1">
                                  <DollarSign className="w-3 h-3" /> Giá / giờ
                                  (VNĐ)
                                </Label>
                                <Input
                                  type="number"
                                  value={zoneForm.priceHour as number}
                                  onChange={(e) =>
                                    setZoneForm((p) => ({
                                      ...p,
                              ...
                              <div className="space-y-1.5">
                                <Label className="text-xs text-green-700 font-semibold flex items-center gap-1">
                                  <DollarSign className="w-3 h-3" /> Giá / ngày
                                  (VNĐ)
                                </Label>
                                <Input
                                  type="number"
                                  value={zoneForm.priceDay as number}
                                  onChange={(e) =>
                                    setZoneForm((p) => ({
                                      ...p,
                                      priceDay: Number(e.target.value),
                                    }))
                                  }
                                />
                              </div>
                            ...
                            <div className="flex gap-2 mt-3 justify-end">
                              ...
                              <Button
                                size="sm"
                                disabled={updateZoneMut.isPending}
                                onClick={() => updateZoneMut.mutate(zoneForm)}
                                className="bg-black hover:bg-slate-800 text-white"
                              >
                                {updateZoneMut.isPending ? (
                                  <Loader2 className="w-3.5 h-3.5 mr-1 animate-spin" />
                                ) : (
                                  <Save className="w-3.5 h-3.5 mr-1" />
                                )}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-slate-800 text-sm">
                                  {zone.zone_name}
                                  <span className="ml-2 text-xs bg-slate-100 border rounded px-1.5 py-0.5 font-mono text-slate-500">
                    ...
                    {addingZone === floor.id ? (
                      <div className="px-5 py-5 bg-blue-50/60 border-t border-blue-100 animate-in slide-in-from-bottom-2 duration-300">
                        <p className="text-xs font-semibold text-blue-700 uppercase mb-3 flex items-center gap-1.5">
                          <Plus className="w-3 h-3" /> Thêm khu vực mới cho{" "}
                          {floor.floor_name}
                        </p>
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1.5">
                              ...
                              {newZoneForm.prefix.length > 0 ? (
                                <>
                                  {[1, 2, 3].map((n) => (
                                    <span
                                      key={n}
                                      className="text-[10px] font-mono font-bold bg-slate-800 text-white px-1.5 py-0.5 rounded"
                                    >
                                      {newZoneForm.prefix}-
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-3 p-3 bg-green-50/80 rounded-xl border border-green-100">
                          <div className="col-span-2">
                            <p className="text-xs font-semibold text-green-700 flex items-center gap-1 mb-2">
                              <DollarSign className="w-3 h-3" /> Thiết lập giá
                              ban đầu
                            </p>
                          </div>
                          <div className="space-y-1.5">
                        ...
                        <div className="flex gap-2 mt-4 justify-end">
                          ...
                          <Button
                            size="sm"
                            disabled={
                              createZoneMut.isPending ||
                              !newZoneForm.zone_name.trim() ||
                              newZoneForm.prefix.length === 0
                            }
                      ...
                      <div className="px-5 py-3 flex justify-center">
                        ...
                        >
                          <Plus className="w-3.5 h-3.5 mr-1.5" /> Thêm khu vực
                          mới vào {floor.floor_name}
                        </Button>
                      </div>
                    )}
                  </div>
