...
export default function Refunds() {
  ...
  const handleReject = async () => {
    ...
    if (
      !window.confirm(
        "Hủy bỏ yêu cầu này? Tiền sẽ được hoàn lại ví người dùng.",
      )
    )
      return;
    setIsProcessing(true);
          ...
          {selectedReq && (
            <div className="my-4 text-sm border border-border rounded-md overflow-hidden bg-background shadow-sm">
              <div className="grid grid-cols-1 sm:grid-cols-3 p-3 border-b border-border gap-1 sm:gap-4 bg-muted/30">
                ...
                <span className="font-mono font-medium text-foreground sm:col-span-2 sm:text-right break-words">
                  {selectedReq.id}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 border-b border-border">
                <span className="text-muted-foreground">Người dùng:</span>
                <span className="font-medium text-emerald-600 dark:text-emerald-400 truncate ml-4">
                  {getUserName(selectedReq.wallet)}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 border-b border-border bg-muted/30">
                ...
                <span className="font-bold text-lg text-emerald-600 dark:text-emerald-500">
                  {Math.abs(selectedReq.amount).toLocaleString("vi-VN")} VNĐ
                </span>
              </div>
                      ...
                      return (
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="mt-4 pt-4 border-t border-border flex flex-col items-center justify-center gap-3">
                            <span className="text-muted-foreground font-semibold text-xs uppercase tracking-wider bg-muted/80 px-3 py-1 rounded-full">
                              Quét QR Tự động điền
                            </span>
                            ...
                            <p className="text-xs text-center text-muted-foreground max-w-[260px] leading-relaxed">
                              ...
                              <span className="font-mono font-medium text-foreground bg-muted p-1 rounded inline-block mt-1">
                                {selectedReq.id}
                              </span>
                            </p>
                          </div>
