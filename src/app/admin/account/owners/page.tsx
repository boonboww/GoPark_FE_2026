  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Quản lý Chủ bãi đỗ</h1>
          <p className="text-gray-500 mt-1">
            Tìm thấy {filteredOwners.length} chủ bãi đỗ
            {usingMockData && (
              <span className="ml-2 text-orange-600 text-sm">(Dữ liệu mẫu - API không khả dụng)</span>
            )}
          </p>
          {error && <p className="text-red-500 text-sm mt-1">Lỗi kết nối: {error}</p>}
        </div>
        <div className="flex gap-3">
          <Button onClick={fetchOwners} variant="outline" className="flex items-center gap-2">
            <RefreshCw size={16} />
            Làm mới
          </Button>
